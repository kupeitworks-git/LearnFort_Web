...
const EditCoaching = () => {
  ...
  const coachingData = location.state?.coaching;
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      coaching_actual_fees_month: coachingData.coaching_actual_fees_month || "",
      coaching_final_fees_month: coachingData.coaching_final_fees_month || "",
      coaching_actual_fees_year: coachingData.coaching_actual_fees_year || "",
      coaching_final_fees_year: coachingData.coaching_final_fees_year || "",
      status: coachingData.status !== undefined ? coachingData.status : true,
    });
    if (coachingData.image) setSportImage(coachingData.image);
    if (coachingData.banner) setBannerImage(coachingData.banner);
    if (coachingData.web_banner) setWebBannerImage(coachingData.web_banner);
  }, [coachingData, navigate]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (payload) => {
      ...
      const res = await fetch(`${BaseUrl}couching/${coachingData._id}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || "Failed to update coaching sport");
      }
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["coaching_admin"] });
      showToast("Coaching sport updated successfully!", "success");
      setTimeout(() => navigate("/coaching"), 1200);
    },
    onError: (err) => showToast(err.message || "Error updating coaching sport", "error"),
  });
        ...
        {preview ? (
          <>
            ...
            <button
              ...
              className="absolute bottom-2 right-2 bg-blue-600 text-white text-xs rounded-lg px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity"
            >
              Change
            </button>
          </>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="0"
            className={`w-full pl-9 pr-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-indigo-50 to-white flex flex-col">
      ...
      <header className="bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] text-white shadow-md sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center">
          ...
          <h1 className="text-xl sm:text-2xl font-semibold tracking-wide">Edit Coaching Sport</h1>
